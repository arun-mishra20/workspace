...
export class HdfcEmailParser implements EmailParser {
  ...
  parseTransactions(email: RawEmail): Transaction[] {
    const details = this.extractDetails(email.bodyText, email.snippet, email.receivedAt);
    ...
    const merchantRaw = normalizeMerchant(details.paidTo);
  ...
  private extractDetails(text: string, snippet: string, receivedAt: string): ParsedHdfcDetails {
    ...
    const transactionMode = this.extractTransactionMode(textLower);
  ...
  private extractTransactionType(textLower: string): TransactionType | null {
    ...
    if (/\b(credited|credit|received|deposited|added to your account|successfully added)\b/i.test(textLower)) {
      return "credited";
    }
    return null;
  }

  private extractTransactionMode(textLower: string): TransactionMode | null {
  ...
  private extractPaidTo(text: string): string | null {
    ...
    for (const pattern of paidToPatterns) {
      ...
      payee = payee.replace(
        /\s+(?:CLIENT\s+ACCO|SEZ|LLP|PVT|LTD|PRIVATE\s+LIMITED)$/i,
        "",
      );
      if (payee.length > 2) {
        return payee;
