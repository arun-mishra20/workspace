...
export class RuleDashboardAnalyticsService {
    ...
    async computeAnalytics(
        ...
        const { rules, missingRuleIds } = await this.ruleDashboardsService.resolveRulesForAnalytics(
            userId,
            request.ruleIds,
            request.inlineRules,
        )
        ...
        const fetchStartDate = [request.startDate, lookbackStartDate].sort()[0]!

        const optimizedTransactions =
            await this.transactionRepository.listByUserInDateRangeMatchingRules({
                userId,
                range: {
        ...
        const transactions =
            ...
            (await this.transactionRepository.listByUserInDateRange({
                userId,
                range: {
                    start: new Date(`${fetchStartDate}T00:00:00.000Z`),
                    end: new Date(`${request.endDate}T23:59:59.999Z`),
        ...
        const byRuleMonthly =
            rules.length > 1 ? groupByRuleMonthly(viewMatchedEntries, rules) : undefined
