...
export class HoldingsService {
    ...
    async createHolding(userId: string, data: CreateHolding): Promise<Holding> {
        ...
        const quantity =
            typeof data.quantity === "string" ? parseFloat(data.quantity) : data.quantity;
        const avgBuyPrice =
            typeof data.avgBuyPrice === "string" ? parseFloat(data.avgBuyPrice) : data.avgBuyPrice;
    ...
    async updateHolding(
        ...
        const avgBuyPrice = data.avgBuyPrice
            ? typeof data.avgBuyPrice === "string"
                ? parseFloat(data.avgBuyPrice)
                : data.avgBuyPrice
            : parseFloat(existing.avgBuyPrice);
        ...
        const currentPrice = data.currentPrice
            ...
            : existing.currentPrice
              ? parseFloat(existing.currentPrice)
              : avgBuyPrice;
    ...
    async importFromGroww(
        ...
        for (const h of parsed) {
            try {
                ...
                skipped++;
                errors.push(`Failed to import ${h.symbol}: ${(err as Error).message}`);
            }
        }
