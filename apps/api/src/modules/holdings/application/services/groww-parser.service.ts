...
import {
    ...
    type GrowwImportData,
    type GrowwEquityHolding,
    type GrowwMutualFundHolding,
    type ParsedGrowwHolding,
...
export class GrowwParserService {
    ...
    private extractMutualFundSymbol(schemeName: string): string {
        ...
        const cleaned = schemeName
            ...
            .replace(/\s*(Growth|Dividend|IDCW)\s*/gi, " ")
            .trim();
        ...
        return (initials || cleaned.substring(0, 10).toUpperCase()) + planHint;
    }
    ...
    parseIndianValue(value: string | number | undefined | null): number {
        ...
        if (/Thousands$/i.test(abs)) {
            return sign * parseFloat(abs.replace(/Thousands$/i, "").trim()) * 1_000;
        }
        if (/Lakhs$/i.test(abs)) {
            return sign * parseFloat(abs.replace(/Lakhs$/i, "").trim()) * 1_00_000;
        }
        if (/Crores$/i.test(abs)) {
            return sign * parseFloat(abs.replace(/Crores$/i, "").trim()) * 1_00_00_000;
        }
